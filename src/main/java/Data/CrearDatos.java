...
public class CrearDatos {
    ...
    public boolean crearArchivo(String Contenido) {// no tocar
        ...
        try {
            // String path = "C:\\Users\\SEBASTIAN MORENO\\OneDrive\\Escritorio\\NetBeans
            // Documents\\TrabajoFinal1\\src\\main\\java\\Datos\\";
            String path = System.getProperty("user.dir") + "\\src\\main\\java\\Datos\\";
            File file = new File(path + nombreArchivo);
            ...
            if (!file.exists()) {
                ...
                // BufferedWriter bufferedWriter = new BufferedWriter(fileWriter); lo comente
    ...
    public static void eliminarArchivo(Scanner scanner) {
        ...
        if (!archivosDisponibles.isEmpty()) { // si existen archivos
            ...
            for (int i = 0; i < archivosDisponibles.size(); i++) {
                System.out.println((i + 1) + ". " + archivosDisponibles.get(i)); // Se muestra la lista de archivos
                                                                                 // disponibles
            }
